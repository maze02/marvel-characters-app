...
export class ComicVineComicMapper {
   ...
   */
  static toDomain(
    response: ComicVineIssueResponse,
   ...
   */
  static toDomainList(
    responses: ComicVineIssueResponse[],
    characterId: CharacterId,
  ): Comic[] {
  ...
  private static cleanHtmlDescription(html: string | null): string {
    ...
    return (
      html
        // Remove HTML tags with space preservation
        .replace(/<[^>]*>/g, " ")
   ...
   */
  private static parseReleaseDate(
   ...
   */
  private static createImageUrl(
    image: ComicVineIssueResponse["image"] | null,
  ): ImageUrl {
    if (!image || !image.medium_url) {
      // Return placeholder for missing images (without extension in path)
      return new ImageUrl(
        "https://via.placeholder.com/200x300/2c3e50/ffffff",
        "png",
      );
    }
