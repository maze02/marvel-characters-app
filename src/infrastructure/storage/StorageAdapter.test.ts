...
describe("StorageAdapter", () => {
  ...
  beforeEach(() => {
    ...
    Object.keys(store).forEach((key) => {
      (mockStorage as Storage & Record<string, string>)[key] = store[key];
    });
  ...
  describe("get", () => {
    ...
    it("should throw StorageError on JSON parse failure", () => {
      ...
      expect(() => adapter.get("bad-data")).toThrow(
        "Failed to read from storage (key: bad-data)",
      );
    });
  ...
  describe("set", () => {
    ...
    it("should throw StorageError on quota exceeded", () => {
      ...
      expect(() => adapter.set("key", "value")).toThrow(
