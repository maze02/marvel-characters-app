...
describe('ComicVineApiClient', () => {
  ...
  beforeEach(() => {
    ...
    (mockedAxios.isAxiosError as any) = (error: any): error is any => error?.isAxiosError === true;
  ...
  describe('error handling', () => {
    it('should throw ApiError for 401 unauthorized', async () => {
      ...
      await expect(client.get('/characters/')).rejects.toThrow('Invalid API key');
    });
    ...
    it('should throw ApiError for 404 not found', async () => {
      ...
      await expect(client.get('/character/4005-999999/')).rejects.toThrow('Resource not found');
    });
    ...
    it('should throw ApiError for 429 rate limit', async () => {
      ...
      await expect(client.get('/characters/')).rejects.toThrow('Rate limit exceeded');
    });
    ...
    it('should throw ApiError for 500 server error', async () => {
      ...
      await expect(client.get('/characters/')).rejects.toThrow('Comic Vine API is currently unavailable');
    });
    ...
    it('should throw ApiError for timeout', async () => {
      ...
      await expect(client.get('/characters/')).rejects.toThrow('Request timeout');
    });
    ...
    it('should throw ApiError for network error', async () => {
      ...
      await expect(client.get('/characters/')).rejects.toThrow('No response from Comic Vine API');
    });
  });
